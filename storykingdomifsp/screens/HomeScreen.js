...
const HomeScreen = () => {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        const fetchedBooks = await fetchRecommendedChildrenBooks();
  ...
  const handleSearch = async () => {
    try {
      let fetchedBooks = await searchBooks(query);
      
      // Aplicar filtro por tema
      if (selectedGenre !== 'All') {
        fetchedBooks = fetchedBooks.filter(book =>
          book.volumeInfo.categories && book.volumeInfo.categories.includes(selectedGenre)
        );
      }
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.header}>Story Kingdom</Text>
